...
BUILD = "prepare_120_core_lagged_seed_ledger_v1"

st.set_page_config(page_title="Prepare 120-Core Lagged Seed Ledger", layout="wide")
st.title("Prepare 120-Core Lagged Seed Trait Ledger")
...
def winner_core_from_result(result4: str) -> str:
    ...
    if not is_valid_double_member(result4):
        return ""
    return "".join(sorted(set(result4)))
...
if uploaded_file is not None:
    ...
    st.download_button(
        label="Download lagged_120_core_seed_trait_ledger.csv",
        data=csv,
        file_name="lagged_120_core_seed_trait_ledger.csv",
        mime="text/csv"
    )
